        ...
        for (Method method : methods) {
            if (method.isAnnotationPresent(MySubscribe.class)) {
                ...
                if (parameterType != null && parameterType.length == 1) {
                    ...
                    MySubscribe sub = method.getAnnotation(MySubscribe.class);
                    int code = sub.code();
    ...
    private void callEvent(SubscriberMethod method, Object object) {
        ...
        if (methods != null && methods.size() > 0) {
            for (SubscriberMethod subscriberMethod : methods) {
                MySubscribe sub = subscriberMethod.method.getAnnotation(MySubscribe.class);
                int c = sub.code();
                if (c == method.code && method.subscriber.equals(subscriberMethod.subscriber)
                        && method.method.equals(subscriberMethod.method)) {
